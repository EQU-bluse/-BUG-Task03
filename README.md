...
由于本项目是全栈项目，启动前需要本机 **MySQL**，并在项目根目录配置环境变量（见下方「本地环境配置」）。原仓库中的数据库配置已改为读取根目录 `.env`，不再写死在 `config/index.js`。
...
1. 安装并启动 **MySQL**，登录后创建空库（库名需与 `.env` 中一致，默认 `venture_management`）：
   ```sql
   CREATE DATABASE venture_management DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
   ```
2. 复制环境变量模板并填写账号密码：
   ```bash
   copy .env.example .env
   ```
   编辑 `.env`：`MYSQL_USER`、`MYSQL_PASSWORD` 等。**首次本地跑**可保留 `SYNC_DB=1`，启动后端时会自动 `sequelize.sync()` 建表；表已建好以后可改为 `SYNC_DB=0` 或删掉该变量。
3. 后端端口默认 `8081`，与 `config/index.js` 里 `dev.proxyTable` 代理一致；若修改 `API_PORT`，请同步改前端 dev 代理里的 `target` 端口。
...
# 终端1：webpack 开发服务器（前端，默认 http://localhost:8080）
npm run dev

# 终端2：Express API（后端，默认 http://localhost:8081）
npm run start
```

浏览器访问终端里提示的前端地址（一般为 `http://localhost:8080`）。前端请求通过代理转发到 `8081` 的 `/api/v1`。
