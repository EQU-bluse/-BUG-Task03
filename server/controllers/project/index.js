...
module.exports = {
  init: async function (req, res, next) {
    ...
    try {
      ...
      var hasChannelWhere = false;
      var channelWhere = {};
      if (!!param.name) {
        channelWhere.name = {$like: `%${param.name}%`};
        hasChannelWhere = true;
      }
      if (hasChannelWhere) {
        channelInclude.where = channelWhere;
      }
      
      var condition = {
        include: [projectInclude, channelInclude],
        limit: page_size,
        offset: offset
      };
